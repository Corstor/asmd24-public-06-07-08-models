...
object Corridor extends App :
  val rl: QMatrix.Facade = Facade(
    ...
    height = 10,
    initial = (0,0),
    terminal = { case (x, 3) if x <= 7 => true; case (7, y) if y >= 3 => true; case (5, 0) => true; case (4, 2) => true; case (8, 1) => true; case (9, 5) => true; case _ => false },
    reward = { case ((9, 9), _) => 100; case ((0, _), LEFT) => -1; case ((_, 0), UP) => -1; case ((9, _), RIGHT) => -1; case ((_, 9), DOWN) => -1; case _ => 0 },
    jumps = Map.empty,
    gamma = 0.9,
    alpha = 0.7,
    epsilon = 0.6,
...
object HideAndSeek extends App:
  val rl: QMatrix.Facade = Facade(
    ...
    initial = (0,0),
    terminal = { case _ => false },
    reward = { case ((0, _), LEFT) => -1; case ((_, 0), UP) => -1; case ((9, _), RIGHT) => -1; case ((_, 9), DOWN) => -1; case _ => 0 },
    jumps = Map.empty,
    gamma = 0.9,
    alpha = 0.7,
    epsilon = 0.8,
